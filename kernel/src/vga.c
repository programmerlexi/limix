...
void draw_ellipse(int xm, int ym, int a, int b, uint32_t color,uint8_t type)
{
    ...
    do
    {
        if (E_3 & type) putpixel(xm + dx, ym + dy, color);
        if (E_4 & type) putpixel(xm - dx, ym + dy, color);
        if (E_1 & type) putpixel(xm - dx, ym - dy, color);
        if (E_2 & type) putpixel(xm + dx, ym - dy, color);
        e2 = 2*err;
