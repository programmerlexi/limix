...
void init_fb(struct limine_framebuffer* fb) {
    g_fb = fb;
}
...
void _internal_putchar(int x, int y, uint8_t ch, uint32_t color, uint8_t h, uint8_t* font) {
    for (uint8_t hi = 0; hi < h; hi++) {
        uint8_t m = 0x80;
        for (uint8_t i = 0; i < 8; i++) {
            if (font[h*ch+hi] & m) putpixel(x+i, y+hi, color);
            m >>= 1;
        }
    }
}
