...
void _smp_start(struct limine_smp_info* cpu_info) {
    ...
    {
        while (cmd->cmd_type != SMP_ACTION_DATA) { asm volatile("nop"); }
    ...
    // Validate data
    if (cpu_info->lapic_id != lapic_id || cpu_info->processor_id != processor_id) {
    ...
    while (cmd->cmd_type != SMP_ACTION_ACK) { asm volatile("nop"); }
    putchar16((cpu_info->processor_id*9), 0, 'P', 0x00ff00);
    cmd->cmd_type = SMP_ACTION_NONE;
    cmd->state = SMP_STATE_READY;

    while (true) {
        if (cmd->cmd_type == SMP_ACTION_NONE)  continue;
        switch (cmd->cmd_type) {
...
void _start(void) {
    ...
    for (uint64_t i = 0; i < smp_request.response->cpu_count; i++) {
        if (smp_request.response->bsp_lapic_id == smp_request.response->cpus[i]->lapic_id) continue;
    ...
    for (uint64_t i = 0; i < smp_request.response->cpu_count; i++) {
        if (smp_request.response->bsp_lapic_id == smp_request.response->cpus[i]->lapic_id) continue;
        smp_cmd_t* cmd = (smp_cmd_t*)&(smp_request.response->cpus[i]->extra_argument);
        ...
        while (cmd->state != SMP_STATE_INIT_WAIT_ACK && cmd->state != SMP_STATE_FROZEN) {
            asm volatile("nop");
            max_cycles--;
            if (max_cycles <= 0) {
                putstr16(0, i * 17, "Timed out", 0xff0000);
